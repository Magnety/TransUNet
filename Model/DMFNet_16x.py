...
class DMFUnit(nn.Module):
    ...
    def __init__(self, num_in, num_out, g=1, stride=1, norm=None, dilation=None):
        ...
        self.conv3x3x3_m2 = DilatedConv3DBlock(num_out, num_out, kernel_size=(1, 3, 3), stride=(1, 1, 1), g=g,
                                               d=(1, 1, 1), norm=norm)
...
class My_DFMNet(nn.Module):
    def __init__(self, in_channel=1, mid_channel=32, channels=128, groups=16, norm='gn', out_classes=2):
        ...
        self.decoder_conv = nn.Conv3d(channels + mid_channel, mid_channel, kernel_size=1, padding=0, stride=1,
                                      bias=False)
...
class DMF_FPN(nn.Module):
    def __init__(self, in_channel=1, mid_channel=32, channels=32, groups=16, norm='gn', out_classes=2):
        ...
        self.decoder_mfunit1 = MFunit(channels *3, channels , g=groups, stride=1, norm=norm)
        self.decoder_mfunit2 = MFunit(4*channels, channels, g=groups, stride=1, norm=norm)
        self.decoder_mfunit3 = MFunit(2*channels, channels, g=groups, stride=1, norm=norm)
        self.decoder_mfunit4 = MFunit(2*mid_channel, channels, g=groups, stride=1, norm=norm)
    ...
    def forward(self, x):
        ...
        encoder4 = self.encoder_block3(encoder3)
        feature3 = self.extra_feature3(encoder4)
        decoder_mfunit1 = self.decoder_mfunit1(encoder4)
        ...
        up1 = self.Up1(decoder_up1)
        att1 = self.att1(g=up1, x=encoder3)
        decoder_cat1 = torch.cat((encoder3, att1), dim=1)
        print("cat1.shape")
        print(decoder_cat1.shape)
        feature4 = self.extra_feature4(decoder_cat1)
        decoder_mfunit2 = self.decoder_mfunit2(decoder_cat1)



        decoder_up2 = F.upsample(encoder3, size=encoder2.size()[2:], mode='trilinear')
        print("22222222222222")
        print(decoder_up2.shape)
        print(encoder2.shape)
        ...
        up2 = self.Up2(decoder_up2)
        att2 = self.att2(g=up2, x=encoder2)
        decoder_cat2 = torch.cat((encoder2, att2), dim=1)
        print("cat2.shape")
        print(decoder_cat2.shape)
        feature5 = self.extra_feature5(decoder_cat2)
        decoder_mfunit3 = self.decoder_mfunit3(decoder_cat2)

        decoder_up3 = F.upsample(encoder2, size=encoder1.size()[2:], mode='trilinear')

        up3 = self.Up3(decoder_up3)
        att3 = self.att3(g=up3, x=encoder1)
        decoder_cat3 = torch.cat((encoder1, att3), dim=1)
        print("cat3.shape")
        print(decoder_cat3.shape)
        decoder_mfunit4 = self.decoder_mfunit4(decoder_cat3)
        concat1 = F.upsample(decoder_mfunit1,size=x.size()[2:],mode='trilinear')
        concat2 = F.upsample(decoder_mfunit2, size=x.size()[2:], mode='trilinear')
        concat3 = F.upsample(decoder_mfunit3, size=x.size()[2:], mode='trilinear')
        concat4 = F.upsample(decoder_mfunit4, size=x.size()[2:], mode='trilinear')
        concat = torch.cat((concat1,concat2,concat3,concat4),dim=1)
        print("concat.shape")
        print(concat.shape)
...
class Deep_Attentive_DMFNet(nn.Module):
    def __init__(self,in_channel=1,mid_channel=32,channels=32,groups=16,norm='gn',out_classes=2):
        ...
        self.decoder_block4 = MFunit(channels + mid_channel ,channels, g=groups, stride=1, norm=norm)
        ...
        self.fc1 = nn.Sequential(nn.Linear(320, 256), nn.Dropout(0.5))
        self.fc2 = nn.Sequential(nn.Linear(256, 128), nn.Dropout(0.5))
        self.fc3 = nn.Sequential(nn.Linear(128, 2), nn.LogSoftmax())
    ...
    def forward(self, x):
        ...
        out_conv_cat = self.decoder_block4(torch.cat((inconv, up_decoder4), 1))
        ...
        #segmentation
        out_conv = self.out_conv(out_conv_cat)
        seg_out = F.upsample(out_conv,size=x.size()[2:], mode='trilinear')
        return inconv, recons1, encoder1, recons2, seg_out, seg_out, cls_out

class Deep_Attentive_DMFNet_32(nn.Module):
    def __init__(self,in_channel=1,mid_channel=32,channels=32,groups=16,norm='bn',out_classes=2):
        super(Deep_Attentive_DMFNet_32, self).__init__()
        self.in_conv = nn.Conv3d(in_channel, mid_channel, kernel_size=3, padding=1, stride=2, bias=False)
        self.encoder_block1 = nn.Sequential(
            DMFUnit(mid_channel, channels, g=groups, stride=2, norm=norm, dilation=[1, 2, 3]),
            DMFUnit(channels, channels, g=groups, stride=1, norm=norm, dilation=[1, 2, 3]),

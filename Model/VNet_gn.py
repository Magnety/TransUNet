...
class LUConv(nn.Module):
    def __init__(self, nchan, elu):
        ...
        self.bn1 = nn.GroupNorm(16,nchan)
...
class DownTransition(nn.Module):
    def __init__(self, inChans, nConvs, elu, stride, dropout=False):
        ...
        self.bn1 = nn.GroupNorm(16,outChans)
...
class UpTransition(nn.Module):
    def __init__(self, inChans, outChans, nConvs, elu, stride, dropout=False):
        ...
        self.bn1 = nn.GroupNorm(16,outChans//2)
        self.do1 = passthrough
...
class ReconsTransition(nn.Module):
    def __init__(self, inChans, outChans,elu):
        ...
        self.bn1 = nn.GroupNorm(16,inChans)
...
class Deep_Attention_block(nn.Module):
    ...
    def __init__(self, F_up, F_down, F_out):
        ...
        self.fuse = nn.Sequential(
            nn.Conv3d(480, 3*F_out, kernel_size=1), nn.GroupNorm(32, 3*F_out), nn.PReLU(),
            nn.Conv3d(3*F_out, 2*F_out, kernel_size=3, padding=1), nn.GroupNorm(32, 2*F_out), nn.PReLU(),
            nn.Conv3d(2*F_out, F_out, kernel_size=3, padding=1), nn.GroupNorm(32, F_out), nn.PReLU()
        )
        ...
        self.out_att = nn.Sequential(
            nn.Conv3d(2*F_out, F_out, kernel_size=1), nn.GroupNorm(32, F_out), nn.PReLU(),
            nn.Conv3d(F_out, F_out, kernel_size=3, padding=1), nn.GroupNorm(32, F_out), nn.PReLU(),
            nn.Conv3d(F_out, F_out, kernel_size=3, padding=1), nn.GroupNorm(32, F_out), nn.PReLU())
        self.PReLU = nn.PReLU()
